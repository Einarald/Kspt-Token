...
function updateNotificationBadges() {
  ...
  if (d.fortuneWheel) {
    ...
    const spinsOk = fw.spinsUsed < 3 || (now - (fw.lastResetTime || 0)) >= cooldownMs;
  ...
  if (d.fortuneWheel) {
    ...
    const spinsOk2 = fw2.spinsUsed < 3 || (now - (fw2.lastResetTime || 0)) >= 50 * 3600 * 1000;
...
function _loadWheelImg(src, cb) {
  ...
  if (_wheelImgCache[src] === true) { cb(null); return; } // failed
  if (_wheelImgCache[src]) { cb(_wheelImgCache[src]); return; }
  const img = new Image();
